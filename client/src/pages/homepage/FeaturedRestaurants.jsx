...
const FeaturedRestaurants = ({ restaurants }) => {
    ...
    const nextSlide = () => {
        if (restaurants && restaurants.length > 0 && !isAnimating) {
            ...
            setCurrentIndex((prevIndex) => {
                ...
                return nextIndex > Math.floor((restaurants.length - 1) / itemsPerSlide) ? 0 : nextIndex;
            });
    ...
    const prevSlide = () => {
        if (restaurants && restaurants.length > 0 && !isAnimating) {
            ...
            setCurrentIndex((prevIndex) => {
                const nextIndex = prevIndex - 1;
                return nextIndex < 0 ? Math.floor((restaurants.length - 1) / itemsPerSlide) : nextIndex;
            });
            setTimeout(() => setIsAnimating(false), 500);
        }
    };
                    ...
                    {loading ? <Skeleton /> :
                        ...
                        >
                            {restaurants && restaurants.length > 0 ? 
                                Array(Math.ceil(restaurants.length / itemsPerSlide))
                                    .fill()
                                    .map((_, slideIndex) => (
                                        <div key={slideIndex} className="flex w-full flex-shrink-0 gap-8">
                                            {restaurants
                                                ...
                                                .map((restaurant) => (
                                                    <div key={restaurant.id} className="w-1/3">
                                                        <LocationCard
                                                            location={restaurant}
