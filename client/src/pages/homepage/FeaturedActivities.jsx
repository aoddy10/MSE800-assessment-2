...
const FeaturedActivities = ({ activities }) => {
    ...
    const nextSlide = () => {
        if (activities && activities.length > 0 && !isAnimating) {
            ...
            setCurrentIndex((prevIndex) => {
                ...
                return nextIndex > Math.floor((activities.length - 1) / itemsPerSlide) ? 0 : nextIndex;
            });
    ...
    const prevSlide = () => {
        if (activities && activities.length > 0 && !isAnimating) {
            ...
            setCurrentIndex((prevIndex) => {
                const nextIndex = prevIndex - 1;
                return nextIndex < 0 ? Math.floor((activities.length - 1) / itemsPerSlide) : nextIndex;
            });
            setTimeout(() => setIsAnimating(false), 500);
        }
    };
                    ...
                    <div className="overflow-hidden">
                        ...
                        >
                            {activities && activities.length > 0 ? 
                                Array(Math.ceil(activities.length / itemsPerSlide))
                                    .fill()
                                    .map((_, slideIndex) => (
                                        <div key={slideIndex} className="flex w-full flex-shrink-0 gap-8">
                                            {activities
                                                ...
                                                .map((activity) => (
                                                    <div key={activity.id} className="w-1/3">
                                                        <LocationCard
                                                            location={activity}
